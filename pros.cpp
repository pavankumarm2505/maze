...
bool DFS(Arrow maze[MAX_ROWS][MAX_COLS], Vertex current, Vertex target, bool visited[MAX_ROWS][MAX_COLS], int rows, int cols, int pathIndex, char path[MAX_ROWS * MAX_COLS][3]) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (next.row >= 0 && next.row < rows && next.col >= 0 && next.col < cols &&
            ...
            path[myPathIndex][0] = directions[i * 2];
            path[myPathIndex][1] = directions[i * 2 + 1];
            path[myPathIndex][2] = '\0';
            (myPathIndex)++;
            if (DFS(maze, next, target, visited, rows, cols, myPathIndex, path)) {
                return true;
            }
            (myPathIndex)--;  // Backtrack
        }
    }
